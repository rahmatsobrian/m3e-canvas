...
export function PartsPalette({
  ...
  /** Touch editors add at the visible screen instead of dragging from a sheet. */
  onPartClick?: (kind: Kind) => void;
}) {
  const lang = useLang();
  ...
  const tile = (k: Kind) => {
    ...
    return (
      <Tile
        ...
        label={labelOf(k)}
        p={p}
        onPointerDown={onPartPointerDown ? (e) => onPartPointerDown(e, k) : undefined}
        onClick={onPartClick ? () => onPartClick(k) : undefined}
