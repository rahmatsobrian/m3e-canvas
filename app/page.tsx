...
export default function Page() {
  ...
  const [sheet, setSheet] = useState<"edit" | "settings" | "lang" | "parts" | "layers" | "color" | "shape" | "type" | "motion" | "ai" | "prompt" | "frame" | "more" | "multi" | null>(null);
  ...
  /* Touch devices keep the complete document and toolset. Their panels move into
     bottom sheets, but screens and every component remain the same as desktop. */
  useEffect(() => {
  ...
  const onCanvasPointerDown = (e: React.PointerEvent) => {
    if (mobileRef.current && e.pointerType === "touch") {
      e.preventDefault();
      /* Capture already promoted this gesture to pinch; do not let the second
         finger bubble through and replace it with a one-finger pan. */
  ...
  /** Grab a frame by its bezel or label. On touch, a tap opens the complete
   *  screen inspector; canvas panning remains available on empty space. */
  ...
  useEffect(() => {
    ...
    if (selectedIds.length < 2 && sheet === "multi") setSheet(null);
  }, [selected, selectedIds.length, sheet]);
  ...
  const tidyTarget = useMemo((): Frame | null => {
    ...
    const g = groups.find((g) => g.items.some((it) => it.id === primaryId));
    return g ? (frameOfGroup(g, frames, widths) ?? null) : null;
  }, [frame, selectedFrame, primaryId, groups, frames, widths]);
  ...
  /** Touch palette: drop any part in the visible part of the selected screen,
   *  keep it inside, and nudge it until it does not cover an existing part. */
  const addMobilePart = (kind: Kind) => {
    const r = canvasRect();
    const v = viewRef.current;
    const item = makeItem(kind);
    const sz = sizeOf(item, widthsRef.current);
    const chosen = framesRef.current.find((candidate) => candidate.id === selectedFrameId);
    const visible = framesRef.current.find((candidate) => {
    ...
    const f = chosen ?? visible ?? framesRef.current[0];
    ...
    if (f) {
      ...
      const isBar = FULL_WIDTH.includes(item.kind);
      placedItem = slot ? (isBar ? carryItemSize(item, { w: PHONE_W, h: PHONE_H }, { w: slot.w, h }) : fitHeight(item, h)) : item;
      const placedSize = sizeOf(placedItem, widthsRef.current);
      const lx = f.x + Math.min(FRAME_MARGIN, (w - placedSize.w) / 2);
      const ly = f.y + Math.min(FRAME_MARGIN, (h - placedSize.h) / 2);
      x = isBar && slot ? slot.x : clamp(x, lx, Math.max(lx, f.x + w - FRAME_MARGIN - placedSize.w));
      y = clamp(y, ly, Math.max(ly, f.y + h - FRAME_MARGIN - placedSize.h));
      const taken = (yy: number) =>
        itemRects().some((o) => o.l < x + placedSize.w && o.r > x && o.t < yy + placedSize.h && o.b > yy);
      let tries = 0;
      while (taken(y) && y + placedSize.h * 2 < f.y + h && tries++ < 12) y += placedSize.h + 12;
    }
    snapshot();
    const dropped: Group = { id: uid(), x: Math.round(x), y: Math.round(y), axis: connectSpecOf(item)?.axis ?? "x", items: [placedItem] };
    setGroups((gs) => [...gs, f ? pullInto(dropped, f, widthsRef.current) : dropped]);
    setSelectedIds([item.id]);
    setSelectedFrameId(null);
    setSheet(null);
  };
  ...
  const addFrame = () => {
    ...
    if (mobileRef.current) setSheet("frame");
  };
      ...
      {drag?.active && (
        ...
        >
          ...
          <AnimatePresence>
            ...
            {isMobile && sheet === "multi" && selectedIds.length > 1 && (
              <BottomSheet key="multi" p={p} onClose={() => setSheet(null)} full>
                <Inspector
                  ai={{ ready: false, reason: aiReason, busy: false, onRun: () => {}, onCancel: cancelAi }}
                  item={null}
            ...
            {isMobile && sheet === "parts" && (
              <BottomSheet key="parts" p={p} onClose={() => setSheet(null)} full>
                <PartsPalette
                  palette={p}
                  favorites={favorites}
                  onToggleFavorite={(k) => setFavorites((f) => f.includes(k) ? f.filter((x) => x !== k) : [...f, k])}
                  onPartClick={addMobilePart}
                />
              </BottomSheet>
            )}
            {isMobile && sheet === "layers" && (
              <BottomSheet key="layers" p={p} onClose={() => setSheet(null)} full>
                <LayersPanel
                  p={p}
                  frames={frames}
                  frameId={layersFrame?.id ?? null}
                  onFrame={(id) => { setLayersFrameId(id); setSelectedIds([]); setSelectedFrameId(id); }}
                  groups={layerGroups}
                  widths={widths}
                  selectedIds={selectedIds}
                  onSelect={(ids, add) => { setSelectedIds((cur) => add ? [...cur.filter((x) => !ids.includes(x)), ...ids] : ids); setSelectedFrameId(null); setSelectedLinkId(null); }}
                  onReorder={reorderLayers}
                  onToggleLock={toggleGroupLock}
                  onReorderItems={reorderGroupItems}
                  onDragging={onLayerDragging}
            ...
            {isMobile && sheet === "prompt" && (
              <BottomSheet key="prompt" p={p} onClose={() => setSheet(null)} full>
                <PromptPanel
                  doc={doc}
                  widths={widths}
                  palette={p}
            ...
            {isMobile && sheet === "frame" && selectedFrame && (
              <BottomSheet key="frame" p={p} onClose={() => setSheet(null)} full>
                <FrameInspector
                  frame={selectedFrame}
                  palette={p}
                  onSize={(preset) => setFramePreset(selectedFrame.id, preset)}
                  onChange={(patch) => patchFrame(selectedFrame.id, patch)}
                  onDelete={() => { deleteFrame(selectedFrame.id); setSheet(null); }}
            ...
            {isMobile && sheet === "color" && (
              <BottomSheet key="color" p={p} onClose={() => setSheet(null)} full>
                <ColorPanel p={p} paletteKey={paletteKey} onPalette={setPaletteKey} custom={customPalette} onCustom={setCustomPalette} dynamic={dynamicColor} onDynamic={setDynamicColor} theme={theme} onTheme={patchTheme} />
              </BottomSheet>
            )}
            {isMobile && sheet === "shape" && (
              <BottomSheet key="shape" p={p} onClose={() => setSheet(null)} full><ShapePanel p={p} theme={theme} onChange={patchTheme} /></BottomSheet>
            )}
            {isMobile && sheet === "type" && (
              <BottomSheet key="type" p={p} onClose={() => setSheet(null)} full><TypePanel p={p} theme={theme} onChange={patchTheme} /></BottomSheet>
            )}
            {isMobile && sheet === "motion" && (
              <BottomSheet key="motion" p={p} onClose={() => setSheet(null)} full><MotionPanel p={p} theme={theme} onChange={patchTheme} /></BottomSheet>
            )}
            {isMobile && sheet === "ai" && (
              <BottomSheet key="ai" p={p} onClose={() => setSheet(null)} full><AiPanel p={p} settings={aiSettings} onSettings={updateAiSettings} /></BottomSheet>
            )}
            {isMobile && sheet === "more" && (
              <BottomSheet key="more" p={p} onClose={() => setSheet(null)}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, padding: "4px 0 12px" }}>
                  {[
                    ["translate", t("language", lang), () => setSheet("lang")],
                    ["palette", t("colors", lang), () => setSheet("color")],
                    ["rounded_corner", t("shape", lang), () => setSheet("shape")],
                    ["text_fields", t("typography", lang), () => setSheet("type")],
                    ["animation", t("motion", lang), () => setSheet("motion")],
                    ["auto_awesome", t("ai", lang), () => setSheet("ai")],
                    ["save", t("saveProject", lang), () => saveProject(doc)],
                    ["file_open", t("openProject", lang), () => projectFileRef.current?.click()],
                    ["share", t("project", lang), () => { setSheet(null); setShareOpen(true); }],
                    ["delete_sweep", t("clearAll", lang), () => { setSheet(null); setConfirmClear(true); }],
                  ].map(([icon, label, onClick]) => (
                    <button key={String(icon)} onClick={onClick as () => void} className="m3-press" style={{ minHeight: 82, padding: 8, border: "none", borderRadius: 18, background: p.surfaceContainerHigh, color: p.onSurface, display: "grid", placeItems: "center", alignContent: "center", gap: 6, fontSize: 11, fontWeight: 600 }}>
                      <Icon name={String(icon)} size={25} color={p.primary} /><span>{String(label)}</span>
                    </button>
                  ))}
                </div>
              </BottomSheet>
